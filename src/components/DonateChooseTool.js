...
function ClsItem({ label, title, paramState, getFocusComp }) {
  ...
  const style = isFocus
    ? 'bg-blue-300 hover:bg-blue-300 hover:text-blue-800 rounded-md cursor-pointer px-5 py-1 flex-shrink-0'
    : 'bg-blue-100 hover:bg-blue-300 hover:text-blue-800 focus:bg-blue-800 rounded-md cursor-pointer px-5 py-1 flex-shrink-0'
  ...
  return (
    <div aria-hidden="true" className={style} onClick={pushParam}>
      {title}
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col text-sm md:text-base space-y-3 px-1 py-4 md:px-20">
      ...
      <div className="flex space-x-3 py-auto overflow-auto">
        ...
        {allClass.map((item) => (
          <ClsItem getFocusComp={getFocusComp} paramState={paramState} label="donationClass" title={item.name} />
        ))}
      </div>
